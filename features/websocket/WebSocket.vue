<template>
    <div class="h-screen flex flex-col justify-between w-[500px]">
        <main class="">
            <pre class="text-xs pt-2 fixed z-10 bg-zinc-100">
          ...
          {{ store.messages.length }} messages
            ...
            <!-- Messages -->
            <div id="messages" class="flex-grow flex flex-col justify-end px-4 py-8 mt-14">
                <div class="flex items-center mb-4" :class="{ 'justify-end': message.user === 'You' }"
                    v-for="message in store.messages">
                    <div class="flex flex-col">
                        <p class="text-gray-500 mb-1 text-xs ml-10" :class="{ 'text-right': message.user === 'You' }">{{
                            message.user }}</p>
                        <div class="flex items-center" :class="{ 'gap-2 flex-row-reverse': message.user === 'You', }">
                            ...
                            <div class="ml-2 text-white bg-gray-800 rounded-lg p-2"
                                ...
                                <p v-if="message.formattedText" class="overflow-x-scroll"
                                    :class="{ '': message.user === 'You' }" v-html="message.formattedText"></p>
            ...
            <!-- Chatbox -->
            <div class="bg-gray-800 px-4 py-2 flex items-center justify-between fixed bottom-0 ">
                ...
                <div class="flex">
                    <button v-if="status === 'CLOSED'" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4"
                        @click="open">
                        Reconnect
                    </button>
                    <button v-if="store.gameId" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4"
                        @click="sendMessage">
                        Send
                    </button>
                    <button v-if="status === 'OPEN' && !store.gameId"
                        class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4" @click="joinGame">
                        Join_Game
                    </button>
                    <button v-if="status === 'OPEN' && store.gameId"
                        class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4" @click="handleClose">
                        Leave
                    </button>
